...
async def main(message, message_text, numeration_command):
	...
	if usage[0] == 1:
		await message.reply("✅ Эту команду больше не нужно использовать. Для упрощения, смысл рулетки был чуть изменён.\n💬 Теперь ты можешь сразу делать любые ставки и крутить рулетку без её запуска.", parse_mode="HTML")
	else:
